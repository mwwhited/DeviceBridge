    ...
    if ((DeviceName != NULL) && (strlen((const char *)DeviceName) < EEFS_MAX_DEVICENAME_SIZE)) {
        ...
        if (DeviceName[0] == '/') {
            ...
            if ((Device = EEFS_GetDevice()) != NULL) {
                ...
                strncpy((char *)Device->DeviceName, (const char *)DeviceName, EEFS_MAX_DEVICENAME_SIZE);
            ...
            if (EEFS_FindDevice(DeviceName) != NULL) {
                ...
                if (EEFS_FindVolume(MountPoint) == NULL) {
                    ...
                    if ((Volume = EEFS_GetVolume()) != NULL) {
                        strncpy((char *)Volume->DeviceName, (const char *)DeviceName, EEFS_MAX_DEVICENAME_SIZE);
                        strncpy((char *)Volume->MountPoint, (const char *)MountPoint, EEFS_MAX_MOUNTPOINT_SIZE);
        ...
        if (strlen((const char *)InputPath) < EEFS_MAX_PATH_SIZE) {
            ...
            if (InputPath[0] == '/') {
                ...
                if (i < strlen((const char *)InputPath)) {
                    ...
                    strncpy((char *)SplitPath->MountPoint, (const char *)InputPath, i);
                    strncpy((char *)SplitPath->Filename, (const char *)&(InputPath[i+1]), EEFS_MAX_PATH_SIZE);
                    ReturnCode = EEFS_SUCCESS;
                }
                else { /* missing second '/' */
                    ReturnCode = EEFS_ERROR;
